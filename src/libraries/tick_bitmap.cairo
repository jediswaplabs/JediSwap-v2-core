...
trait ITickBitmap<TState> {
    fn next_initialized_tick_within_one_word(ref self: TState, tick: i32, tick_spacing: u32, search_left: bool) -> (i32, bool);
...
mod TickBitmapComponent {
    ...
    impl TickBitmapImpl<TContractState, +HasComponent<TContractState>> of super::ITickBitmap<ComponentState<TContractState>> {
        ...
        fn next_initialized_tick_within_one_word(ref self: ComponentState<TContractState>, tick: i32, tick_spacing: u32, search_left: bool) -> (i32, bool) {
            let tick_spacing_i = IntegerTrait::<i32>::new(tick_spacing, false);
            let mut compressed: i32 = tick / tick_spacing_i;
            if ((tick < IntegerTrait::<i32>::new(0, false)) && (tick % tick_spacing_i != IntegerTrait::<i32>::new(0, false))) {
            ...
            if (search_left) {
                ...
                let next = if (initialized) {
                    (compressed - (bit_pos - BitMath::most_significant_bit(masked)).into()) * tick_spacing_i 
                    } else {
                ...
                // overflow/underflow is possible, but prevented externally by limiting both tickSpacing and tick
                let next = if initialized {
                    (compressed + IntegerTrait::<i32>::new(1, false) + (BitMath::least_significant_bit(masked) - bit_pos).into()) * tick_spacing_i
                    } else {
                    (compressed + IntegerTrait::<i32>::new(1, false) + (BoundedInt::<u8>::max() - bit_pos).into()) * tick_spacing_i
                    };
                return (next, initialized);
            }
        }
    ...
    impl InternalImpl<TContractState, +HasComponent<TContractState>> of InternalTrait<TContractState> {
        fn is_initialized(ref self: ComponentState<TContractState>, tick: i32) -> bool {
            let (next, initialized) = TickBitmapImpl::next_initialized_tick_within_one_word(ref self, tick, 1, true);
            if next == tick {
                initialized
            } else {
                false
