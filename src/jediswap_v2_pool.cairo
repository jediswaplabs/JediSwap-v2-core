...
mod JediSwapV2Pool {
    ...
    struct Storage {
        ...
        tick_spacing: i32,          // @notice The pool tick spacing
        ...
        tick: i32,                  // @notice The current tick of the pool, i.e. according to the last tick transition that was run.
                                    ...
                                    // boundary.
        fee_protocol: u8,           // @notice The protocol fee for both tokens of the pool.
    ...
    #[constructor]
    fn constructor(ref self: ContractState, token0: ContractAddress, token1: ContractAddress, fee: u32, tick_spacing: i32) {
    ...
    impl JediSwapV2PoolImpl of super::IJediSwapV2Pool<ContractState> {
        ...
        fn initialize(ref self: ContractState, sqrt_price_X96: u256) {
            ...
            assert(self.sqrt_price_X96.read().is_zero(), 'AI');
        ...
        fn swap(ref self: ContractState, recipient: ContractAddress, zero_for_one: bool, amount_specified: i256, sqrt_price_limit_X96: u256, data: Array<felt252>) -> (i256, i256) {
            ...
            let fee_protocol_start = self.fee_protocol.read();
            ...
            let fee_protocol = if (zero_for_one) {
                    ...
                    fee_protocol_start % 16
                } else {
                    // calculate feeProtocol1
                    fee_protocol_start.shr(4)
                };
    ...
    impl InternalImpl of InternalTrait {
        ...
        fn balance0(self: @ContractState) -> u256 {
            let token_dispatcher = IERC20Dispatcher { contract_address: self.token0.read() };
            token_dispatcher.balance_of(get_contract_address()) //TODO which balance_of
        }

        fn balance1(self: @ContractState) -> u256 {
            let token_dispatcher = IERC20Dispatcher { contract_address: self.token1.read() };
            token_dispatcher.balance_of(get_contract_address()) //TODO which balance_of
        }
    }
